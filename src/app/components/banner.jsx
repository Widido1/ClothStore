    ...
    return(
        <div className="grid grid-flow-row theme3 place-self-center text-center justify-center items-center w-full px-2 font-bold italic text-3xl
            ...
            <div className="sm:w-1/3">
                <h1 className="Shine text-3xl min-[290px]:text-5xl"><Link href="/">CozyBoys</Link></h1>
            </div>
            ...
            text-xs min-[290px]:text-sm sm:text-base sm:w-2/3 md:text-lg lg:text-xl">
                <button className="Shine p-2"><Link href="/about">About</Link></button>
                {/*Esta logica renderiza acorde a si el usuario esta autenticado y que permisos tiene*/}
                
                {requiredPermission.isGranted && (
                    <button className="Shine p-2"><Link href="/new-product">New Product</Link></button>
                )}

                {!isAuthenticated ? (
                    <LoginLink className="Shine p-2">Log in</LoginLink>
                ):( 
                <div className="flex p-2">
                    ...
                    <div className="flex-2 flex-row justify-items-start p-2">
                        ...
                        <LogoutLink className="Shine text-right text-xs min-[600px]:text-sm">Log Out</LogoutLink>
                    </div>
                </div>
